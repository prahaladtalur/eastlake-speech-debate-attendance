    ...
    <main className="min-h-screen bg-[#f6f7f8] text-slate-950">
      ...
      <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
        <div className="flex flex-col gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Passing is {TARGET_PERCENT}% or higher.
            </p>
          </div>
        ...
        <div className="mt-7 grid gap-8 lg:grid-cols-[minmax(0,1fr)_320px]">
          <section aria-labelledby="attendance-heading" className="min-w-0">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 id="attendance-heading" className="text-lg font-semibold">
                  Who&apos;s here?
                </h3>
                <p className="mt-1 text-sm text-slate-500">
                  Select Here or Away, then save.
                </p>
              </div>
              <span className="shrink-0 text-sm font-medium text-slate-600">
                {presentCount} / {eligibleMembers.length} here
              </span>
            </div>

            {loading ? (
              <div className="mt-5 space-y-4" aria-label="Loading attendance" role="status">
                {[1, 2, 3].map((item) => (
                  <div key={item} className="h-12 animate-pulse border-b border-slate-200 bg-slate-100/70" />
                ))}
              </div>
            ) : eligibleMembers.length === 0 ? (
              <div className="mt-5 border-y border-slate-200 py-10 text-sm text-slate-500">
                Add people in the roster section to start taking attendance.
              </div>
            ) : (
                ...
                {eligibleMembers.map((member) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className="flex shrink-0 rounded-lg border border-slate-200 p-0.5"
            ...
            <Button
              type="button"
              size="lg"
              className="mt-5 w-full bg-[#e8652b] text-white hover:bg-[#d95822]"
            ...
            <p className="mt-2 text-xs text-slate-500">
              {selectedMeeting
                ? "You are editing this saved date."
                : "This date will be added when you save."}
            </p>
          </section>
          ...
          <aside className="space-y-8">
            <section aria-labelledby="roster-heading">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h3 id="roster-heading" className="text-lg font-semibold">
                    Add people
                  </h3>
                  <p className="mt-1 text-sm text-slate-500">Everyone with the link can add someone.</p>
                </div>
                <Users className="mt-1 size-5 text-slate-400" aria-hidden="true" />
              </div>
              ...
              <ul className="mt-5 divide-y divide-slate-200 border-y border-slate-200">
                {members.length === 0 ? (
                  <li className="py-4 text-sm text-slate-500">No people added yet.</li>
                ) : (
                  memberStats.map((stat) => (
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="mt-0.5 text-xs text-slate-500">
                          {stat.present} / {stat.eligibleMeetings} · since {formatShortDate(stat.member.eligibleFrom)}
                        </p>
                      </div>
                      <span className={cn("shrink-0 text-right text-sm font-semibold", statusTone(stat.percent))}>
                        <span className="block">{stat.percent === null ? "—" : `${stat.percent}%`}</span>
            ...
            <section aria-labelledby="history-heading">
              ...
              {meetings.length === 0 ? (
                <p className="mt-3 text-sm text-slate-500">No saved dates yet.</p>
              ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="flex items-center gap-2 text-slate-500">
                          {sessionPresentCount(meeting)} / {total}
                          <Button
                            type="button"
                            size="sm"
                            variant="ghost"
        ...
        <p className="mt-8 border-t border-slate-200 pt-4 text-xs leading-5 text-slate-500">
          Anyone with this link can update the shared record. New members start counting from the day they are added.
        </p>
      </div>
    </main>
  );
